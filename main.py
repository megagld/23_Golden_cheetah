...
def get_json_file(video, GC_routes_files):

    # http://cdn.virtualtraining.eu/routes/93871/profile/geometry.json

    adres = f'http://cdn.virtualtraining.eu/routes/{video}/profile/geometry.json'

    solditems = requests.get(adres)
    data = solditems.json()
    with open(f'{GC_routes_files}/{video}.json', 'w') as f:
        json.dump(data, f)
...
GC_routes_files = 'D:/_Golden Cheetach/_json'
...
for video in video_file_list:
    if video in GC_file_list:
        ...
        if video not in GC_routes_list:
            get_json_file(video, GC_routes_files)   
        make_files(video,
                   main_video_folder,
                   main_gpx_folder,
                   GC_routes_files)
...
pymsgbox.alert(text='Utworzone pliki:   {}'.format('; '.join(utworzone_pliki)), title='{}'.format('GC'), button='OK')
