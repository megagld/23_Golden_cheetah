...
class Trace:
    ...
    def calc_length_gain(self):
        self.length = int(self.geometry_points[-1].distance)
        self.gain = 0

        previous_altitude = None
        for i, point in enumerate(self.geometry_points):
            if i > 0:
                if point.alt > previous_altitude:
                    self.gain += point.alt - previous_altitude
            previous_altitude = point.alt
    ...
    def set_location(self):
        ...
        coordinates = f"{self.GC_points[0].lat}, {self.GC_points[0].lon}"
        location = locator.reverse(coordinates, language='pl')
        ...
        for lower_id in ['city', 'village','town','state']:
            try:
                lower = location.raw['address'][lower_id]
            except:
                continue
            break

        self.location = f'{country}_{lower}'
    ...
    def make_GC_file(self, GC_file_path, start_time):
        ...
        GC_file_path = GC_file_path.replace('.json',f'_{self.location}_{self.length//1000}_{self.gain}.json')
